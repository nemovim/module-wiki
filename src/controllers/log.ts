...
import type { DocLogDoc, DocLog, UserLogDoc, UserLog, PenaltyLog, PenaltyLogDoc } from '../types/log';
...
export default class LogController {
    ...
    static async getDocLogsByUserName(userName: UserName, limit: number, skip = 0): Promise<Array<DocLogDoc>> {
        return await DocLogModel.find({
            userName,
            fullTitle: {
                $not: /^숨김:.+/,
