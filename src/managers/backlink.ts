...
export default class BacklinkManager {
    static async createBacklinkMarkupByFullTitle(fullTitle: string): Promise<string|null> {
        ...
        if (!backlink)
            return null;
        let content = '';
        content += MarkupUtils.createAlignedMarkupByFullTitleArr(backlink.linkedFromArr, '이 문서가 연결된');
        content += MarkupUtils.createAlignedMarkupByFullTitleArr(backlink.embeddedInArr, '이 문서가 삽입된');
        content += MarkupUtils.createAlignedMarkupByFullTitleArr(backlink.redirectedFromArr, '이 문서로 이동되는');
        return content;
    }
    ...
    static async updatelinksByDocs(prevDoc: Partial<Doc>&{markup: string} | null, nextDoc: Partial<Doc>&{fullTitle: string, markup: string}): Promise<void> {
        ...
        const prevEmbedArr = TitleUtils.setPrefixToTitleArr(WikiTranslator.getFileTitleArr(prevDoc?.markup||''), '파일');
        const nextEmbedArr = TitleUtils.setPrefixToTitleArr(WikiTranslator.getFileTitleArr(nextDoc.markup), '파일');
