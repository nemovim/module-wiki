...
export default class MarkupUtils {
    ...
    static createMarkupByChoseongMap(choseongMap: Map<string, string[]>, prefix: string): string {
        ...
        choseongArr.forEach(choseong => {
            const parsedTitleArr: string[] = (choseongMap.get(choseong) || []).map(title => WikiTranslator.toEscape(title));
            middlePart += `(${choseong}\n:([[ `;
    ...
    static createAlignedMarkupByFullTitleArr(fullTitleArr: string[], beforeStr: string=''): string {
        let markup = '';
        ...
        prefixArr.forEach((prefix, i) => {
            ...
            markup += MarkupUtils.createMarkupByChoseongMap(
                choseongMap,
                prefix
            );
