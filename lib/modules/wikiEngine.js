...
class WikiEngine {
    ...
    constructor(
        ...
        CATEGORIZED_TITLE = '분류',
        UNCATEGORIZED_TITLE = '미분류'
    ) {
        this.#wikiDB = new WikiDB(MONGO_URI);
        this.#wikiManager = new WikiManager(TYPE_OBJ);
    ...
    async init() {
        try {
            ...
            if (DOC_ARR.length === 0) {
                ...
                console.log('[Initiating Wiki Categorizer...]');
                const CATEGORIZED_ID = this.#wikiManager.getIdByFullTitle(this.#wikiManager.getFullTitle('category', this.#wikiCategorizer.categorizedTitle));
                const UNCATEGORIZED_ID = this.#wikiManager.getIdByFullTitle(this.#wikiManager.getFullTitle('category', this.#wikiCategorizer.uncategorizedTitle));
                this.#wikiCategorizer.init(CATEGORY_ARR, CATEGORIZED_ID, UNCATEGORIZED_ID);
                return true;
            }
        } catch (e) {
            throw new Error(e);
