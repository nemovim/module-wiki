...
class WikiEngine {
    ...
    async writeDoc(fullTitle, content, author, comment) {
        try {
            ...
            let createDocArr =
                this.#wikiManager.createCategoriesByIdArr(createCategoryIdArr, this.#wikiCategorizer.uncategorizedTitle);
            let deleteDocArr =
    ...
    async readDoc(fullTitle) {
        try {
            ...
            doc.fullTitle = this.#wikiManager.getFullTitleById(doc.id);

            if (doc.type === 'category') {
                let categorizedArr = this.#wikiCategorizer.getCategorizedIdArr(doc);
                doc.categorized.set('doc', categorizedArr.reduce((prev, id) => {
                    if (this.#wikiManager.getTypeById(id) !== 'category') {
                        prev.push(this.#wikiManager.getFullTitleById(id));
                    }
                    return prev;
                }, []));
                doc.categorized.set('category', categorizedArr.reduce((prev, id) => {
                    if (this.#wikiManager.getTypeById(id) === 'category') {
                        prev.push(this.#wikiManager.getTitleById(id));
                    }
                    return prev;
                }, []));
            }
    ...
    async previewDoc(content) {
        try {
            return WikiTranslator.translate(content);
        } catch (e) {
            throw new Error(e);
        }
    }
    ...
    searchDoc(searchWord) {
        ...
        return hangulSearcher.search(searchWord, 0.2);
    }
