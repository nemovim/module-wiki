...
export default class CategoryManager {
    static async createCategoryMarkupByCategorizedArr(categorizedArr) {
        ...
        const prefixArr = Utils.moveOrSetItemToFirstInArr(
    ...
    createChosungMapByTitleArr(titleArr) {
        // chosung -> titleArr
        ...
        chosungMap.keys().forEach((key) => {
            chosungMap.get(key).sort();
        });

        return chosungMap;
    }

    createCategoryMarkupByChosungMap(chosungMap, prefix) {
        // '\n:' + '(categorized)*keys' + ':'
        return (
            '\n:' +
            chosungMap.keys().reduce((prev, chosung) => {
                prev += `(${chosung}\n:([[`;
