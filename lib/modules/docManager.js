...
export default class DocManager {
    static createDocByInfo(info, user={name: 'anonymous'}, markup='', comment='') {
        const doc = {
            docId: info.docId,
            type: info.type,
    ...
    static createNextDocByPrevDoc(prevDoc, user, markup, comment, state='normal') {
    ...
    // static async getDocByDocId(docId, revision = -1) {
    //     const info = await InfoManager.getInfoByDocId(docId);
    //     if (info === null) {
    //         return null;
    //     } else {
