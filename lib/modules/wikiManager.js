...
class WikiManager {
    ...
    createDoc(title, content, author, comment) {
        ...
        let { updateCategoryMap, newCategoryMap } = this.categorizeDoc(
            docArray[0]
        );
    ...
    createNewDoc(title, content, author, comment, categorizedArray = []) {
        ...
        if (categorizedArray.length !== 0) {
            ...
            newDoc = new WikiCategory(params, categorizedArray);
        } else {
            console.log('Create new doc');
            newDoc = new WikiDoc(params);
        }
    ...
    createNextDoc(title, content, author, comment) {
        ...
        if (this.isCategory(title)) {
            return new WikiCategory(params, this.#categoryMap.get(ID));
        } else {
            return new WikiDoc(params);
        }
    }
